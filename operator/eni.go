...
func lookupPodIP(ip string) *v1.Pod {
...
func makeExistingENIsAvailable(enis []v2.ENI, node *v2.CiliumNode) {
	...
	for _, e := range enis {
		...
		if e.Number < node.Spec.ENI.FirstAllocationInterface {
			log.Debug("Ignoring ENI %s due to %d < %d", e.Number, node.Spec.ENI.FirstAllocationInterface)
			continue
		}

		for _, ip := range e.Addresses {
			if _, ok := node.Spec.IPAM.Available[ip]; !ok {
				if pod := lookupPodIP(ip); pod != nil {
					node.Status.IPAM.InUse[ip] = v2.AllocationIP{
						PodName:  pod.Name,
						Resource: e.ID,
					}
				} else {
					node.Spec.IPAM.Available[ip] = v2.AllocationIP{
						Resource: e.ID,
					}
				}
			}
...
func refreshNode(enis []v2.ENI, node *v2.CiliumNode) error {
	...
	availableIPs := len(node.Spec.IPAM.Available)
	usedIPs := len(node.Status.IPAM.InUse)
	needed := requiredAddresses - (availableIPs - usedIPs)
	if needed > 0 {
		log.Infof("Need to allocate %d additional IPs for instance %s", needed, node.Spec.ENI.InstanceID)
	}
...
func startENIAllocator() error {
	...
	go func() {
		...
		for _, obj := range podStore.List() {
			if pod, ok := obj.(*v1.Pod); ok {
				log.Infof("%s: %s", pod.Name, pod.Status.PodIP)
			}
		}
	}()
